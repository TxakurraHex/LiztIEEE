...
\begin{abstract}
  % ============================================================
  % Target: ~200 words. Write this LAST once all sections are done.
  % Cover: problem, approach, key results, contributions.
  %
  % Placeholder:
  Software composition analysis (SCA) tools help developers identify vulnerable dependencies, but existing approaches relying solely on CPE/CVE package matching produce substantial false positives by flagging packages whose vulnerable code paths are never executed in practice. We present \textit{Lizt}, an open-source, eBPF-based runtime reachability scanner that layers uprobe monitoring on top of static CPE matching to confirm whether vulnerable symbols are actually reached during system execution. By deferring a finding's severity until runtime evidence confirms reachability, Lizt reduces actionable noise without sacrificing coverage. We describe Lizt's five-stage pipeline, its implementation in Rust using \texttt{aya} for eBPF and \texttt{axum} for the findings dashboard, and evaluate it against a suite of source-compiled fixtures spanning zlib, libexpat, and OpenSSL. Across 73 CPE-matched CVEs, Lizt reclassifies 78\% as lacking runtime evidence and surfaces 16 (21.9\%) as observed during fixture workloads.
\end{abstract}
...
\begin{itemize}[noitemsep]
  \item A five-stage pipeline combining inventory collection, CVE ingestion, symbol extraction, eBPF runtime monitoring, and a ranked findings dashboard.
  \item A confidence-tier binary analysis system that supports Rust (via \texttt{rust-audit-info}), Go (module extraction), and arbitrary ELF binaries (strings-based fallback).
  \item An evaluation fixture framework for reproducible testing against known vulnerability scenarios.
  \item An open-source implementation in Rust using \texttt{aya}~\cite{aya}, \texttt{axum}, \texttt{sqlx}, and PostgreSQL.
\end{itemize}
